"""First real run of faultline.solver.run_cascade, on the same floor-shock
scenario as the hand-walk and regression test. Confirms the built solver
reproduces (or, per the close-factor correction, correctly DIFFERS from)
the hand-verified reference numbers -- see BUILDLOG.md, 2026-09-05,
"Recomputing the cascade test with the correct close factor" and the
solver-run entry after it.
"""
...
from faultline.uniswap import human_scale_liquidity
...
WALLETS = [
    "0x86aef245207e2f93fba083d51852c91a7e711eb6",  # primary (trigger)
    "0xcc997fe9fdd5957ca3060af68318f754d75016d3",  # genuine pool-impact-only victim
]

WSTETH_FLOOR = 1718.7752
# Infinitesimally below the exact floor. The floor is defined as the price
# where the primary wallet's HF equals exactly 1.0 -- the boundary of
# liquidatable, not liquidatable itself (Aave requires HF < 1). A live
# re-fetch could land on either side of 1.0 by a tiny margin depending on
# accrual since the floor was computed; nudging guarantees the scenario
# actually triggers rather than depending on which side of an exact
# floating-point boundary a fresh query happens to fall on.
SHOCK_START_PRICE = WSTETH_FLOOR * (1 - 1e-6)

if __name__ == "__main__":
    wallets = {w: aave.fetch_wallet_positions(w) for w in WALLETS}

    # Phase-A pool state: already walked to the 43.6740% shocked price by
    # arbitrage before any wallet liquidation begins (BUILDLOG.md, "Phase
    # A"). Reusing that end state here rather than re-deriving it --
    # same 85-tick crossing, same real tick data.
    ...
    print(f"liquidated: {result['liquidated_wallets']}")
    print()
